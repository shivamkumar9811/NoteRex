    ...
    const normalizeNoteData = () => {
      ...
      if (parsedData) {
        ...
        if (parsedData.summaryFormats) {
          summaryFormats = {
            bulletNotes: Array.isArray(parsedData.summaryFormats.bulletNotes) ? parsedData.summaryFormats.bulletNotes : [],
            topicWise: Array.isArray(parsedData.summaryFormats.topicWise) ? parsedData.summaryFormats.topicWise : [],
            keyTakeaways: Array.isArray(parsedData.summaryFormats.keyTakeaways) ? parsedData.summaryFormats.keyTakeaways : [],
          };
        }
        if (Array.isArray(parsedData.revisionQA)) {
          revisionQA = parsedData.revisionQA.filter((i) => i && (i.question || i.q));
        }
      }
      ...
      if (summaryFormats.bulletNotes.length === 0 && currentPayload.summaries) {
        ...
        if (summaries.keyTakeaways) {
          summaryFormats.keyTakeaways = Array.isArray(summaries.keyTakeaways) ? summaries.keyTakeaways : summaries.keyTakeaways.split('\n').filter(Boolean);
        }
        if (summaries.qa && Array.isArray(summaries.qa)) {
          revisionQA = summaries.qa.filter((i) => i && (i.question || i.q));
        }
      }

      // 5. Reject empty summaries - never save {}
      if (summaryFormats.bulletNotes.length === 0 && summaryFormats.topicWise.length === 0 && summaryFormats.keyTakeaways.length === 0) {
        throw new Error('No summaries generated. Please try again with different content.');
      }
...
/** Centralized on-demand.io agent call for use from UI (e.g. chat) */
export { callAgent as callOnDemandAgent };
